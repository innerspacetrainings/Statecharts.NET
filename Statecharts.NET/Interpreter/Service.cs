...
namespace Statecharts.NET.Interpreter
{
    ...
    internal static class TransitionFunctions
    {
        internal static TResult Map<TContext, TResult>(
            this Transition<TContext> transition,
            Func<EventlessTransition<TContext>, TResult> fEventless,
            Func<EventfulTransition<TContext>, TResult> fEventful)
            where TContext : IEquatable<TContext>
        {
            switch (transition)
            {
                case EventlessTransition<TContext> eventless:
                    return fEventless(eventless);
                case EventfulTransition<TContext> eventful:
                    return fEventful(eventful);
                default: throw new Exception("NON EXHAUSTIVE SWITCH");
            }
        }
    }
        ...
        where TContext : IEquatable<TContext>
    {
        ...
        public Transition(BaseStateNode<TContext> source, IEnumerable<BaseStateNode<TContext>> targets)
        {
            Source = source ?? throw new ArgumentNullException(nameof(source));
            Targets = targets ?? throw new ArgumentNullException(nameof(targets));
        }
    }
    internal class EventlessTransition<TContext> : Transition<TContext>
        where TContext : IEquatable<TContext>
    {
        public IEnumerable<Action> Actions { get; }

        public EventlessTransition(BaseStateNode<TContext> source, IEnumerable<BaseStateNode<TContext>> targets,
            IEnumerable<Action> actions) : base(source, targets)
        {
            Actions = actions ?? throw new ArgumentNullException(nameof(actions));
        }
    }
    internal class EventfulTransition<TContext> : Transition<TContext>
        where TContext : IEquatable<TContext>
    {
        public IEnumerable<EventAction> Actions { get; }

        public EventfulTransition(BaseStateNode<TContext> source, IEnumerable<BaseStateNode<TContext>> targets,
            IEnumerable<EventAction> actions) : base(source, targets)
        {
            Actions = actions ?? throw new ArgumentNullException(nameof(actions));
        }
    }
